% References:
% "On the sphere-decoding algorithm I. Expected complexity," in TSP, Aug. 2005
% MIMO sphere decoder
% works for 2-PAM only (it can be very easily modified for higher PAM)

function dec_seq = fpSD(H,y,N0)
	[Nt,Nr] = size(H); % number of transmit antennas
	%Nr = 8; % number of receive antennas
	fade_var = 1; % fade variance of the MIMO channel
	SNR_dB = 8; % SNR per bit (dB)
	epsilon = 0.000001; % for choosing radius
	num_frames = 10^2; % simulation runs

	% SNR parameters
	SNR = 10^(0.1*SNR_dB);
	noise_var = N0; % 1*fade_var*Nt*Nr/(2*SNR*1*Nt); %awgn variance

	% radius (square) of the sphere
	d1_2 = Inf; %chi2inv(1-epsilon,Nr)*noise_var;
	...
	fade_chan = H; % normrnd(0,sqrt(fade_var),Nr,Nt);

	% AWGN
	%noise = normrnd(0,sqrt(noise_var),Nr,1);

	% channel output
	chan_op = y; % fade_chan*seq.' + noise;
	...
	switch (caseno)
		...
		case 2
			 ...
			 seq2(k) = low_bound-1;   % start from minimum
			 caseno=3;  
		case 3
			 seq2(k) = seq2(k)+1;
